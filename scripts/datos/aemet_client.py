...
class AEMETClientRobusto:
        ...
        while intentos < max_intentos:
            try:
                ...
                response = requests.get(
                    url, headers=self.headers, params=params, timeout=timeout_actual
                )
                ...
                logging.warning(
                    f"⏰ Timeout en intento {intentos + 1}, reintentando..."
                )
                intentos += 1
                time.sleep(5)
    ...
    def _datos_estacion_simulados(self, codigo_estacion):
        ...
        datos_por_estacion = {
            ...
            "5675": {
                "temperatura": 24.1,
                "humedad": 60,
                "viento_velocidad": 12.8,
            },  # Jerez
            "6000": {
                "temperatura": 21.8,
                "humedad": 70,
                "viento_velocidad": 18.2,
            },  # Tarifa
            "5785": {
                "temperatura": 23.2,
                "humedad": 62,
                "viento_velocidad": 14.1,
            },  # Huelva
        }

        datos_base = datos_por_estacion.get(
            codigo_estacion,
            {"temperatura": 22.0, "humedad": 65, "viento_velocidad": 15.0},
        )
        ...
        return {
            ...
            "humedad": max(
                40, min(90, datos_base["humedad"] + random.randint(-10, 10))
            ),
            "viento_velocidad": max(
                0, datos_base["viento_velocidad"] + random.uniform(-5, 5)
            ),
    ...
    def verificar_conexion(self):
        ...
        if resultado:
            ...
            logging.warning(
                f"❌ AEMET No disponible (timeout después de {duracion:.1f}s)"
            )
            return False
    ...
    def obtener_resumen_alertas(self):
        ...
        for nombre, datos_estacion in datos["estaciones"].items():
            ...
            if datos_estacion.get("viento_velocidad", 0) > 50:  # km/h
                alertas.append(
                    f"🌬️ Viento fuerte en {nombre}: {datos_estacion['viento_velocidad']} km/h"
                )

            if datos_estacion.get("temperatura", 0) > 35:
                alertas.append(
                    f"🌡️ Temperatura alta en {nombre}: {datos_estacion['temperatura']}°C"
                )
...
def probar_conexion_aemet():
    ...
    if client.verificar_conexion():
        ...
        if datos:
            print(
                f"✅ Datos de Cádiz: {datos['temperatura']}°C, {datos['viento_velocidad']} km/h"
            )
