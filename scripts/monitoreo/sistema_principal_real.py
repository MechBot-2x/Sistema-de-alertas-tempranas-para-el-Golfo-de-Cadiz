...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/sistema_real.log"), logging.StreamHandler()],
)
    ...
    def verificar_datos_reales(self):
        ...
        try:
            ...
            fuentes = {
                "sismos": (
                    "real"
                    if any(s.get("lugar", "").find("Simulado") == -1 for s in sismos)
                    else "simulado"
                ),
                "boyas": (
                    "real"
                    if any(b.get("fuente") == "real" for b in boyas)
                    else "simulado"
                ),
                "mareas": (
                    "real"
                    if mareas.get("cadiz", {}).get("fuente") == "real"
                    else "simulado"
                ),
            }

            logging.info(
                f"✅ Datos obtenidos: {len(sismos)} sismos ({fuentes['sismos']}), "
        ...
        try:
            ...
            if datos_reales:
                ...
                if analisis_riesgo["riesgo"] > 0.6:
                    mensaje = (
                        f"🚨 ALERTA {analisis_riesgo['nivel']}: "
                        f"Riesgo {analisis_riesgo['riesgo']:.2f}\n"
